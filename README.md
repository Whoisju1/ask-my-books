...
The corpus is *No More Magic*, a full-length technical book. To be clear about its provenance: it was generated by an AI model at my direction — I set the premise, outline, and scope — rather than written by me. What it gives this project is a realistic, book-length body of prose to retrieve against: chapters, sections, and a consistent voice, with no licensing question about indexing it. It isn't in this repository. The ingest CLI reads from a local `data/` directory that's gitignored, the test suite runs against a small fixture corpus, and CI never needs the real book or an API key.
